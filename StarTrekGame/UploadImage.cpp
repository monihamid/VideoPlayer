...
* \return Nothing
*/
void UploadImage::DrawImage(float left, float top, float angle)
{
	float width = GetBitmap()->GetSize().width;
	float height = GetBitmap()->GetSize().height;

	D2D1_POINT_2F centerOfImage = D2D1::Point2F(
		left + width / 2,
		top + height / 2);

	gfx->GetRenderTarget()->SetTransform(
		D2D1::Matrix3x2F::Rotation(angle, centerOfImage)
	);
