...
Movement::Movement(float width, float height)
{
	...
	shipDStart.first = currentPositionPS.first;                   //0;                  //direction of shipbase
	shipDStart.second = currentPositionPS.second;                               //384;
...
/**
* \brief Random move of the head.
* \details Calls upon a D2D1 method to specify the location of image file.
* \param: pair startPoints
*         pair endP
* \return Nothing
*/
void Movement::RandomMove(pair <float, float> startPoints)
{
	pair <float, float> endPoint;

	if (startPoints.first >= 500 && startPoints.second >= 500)
	{
		startPoints.first= startPoints.first--;
		startPoints.second = startPoints.second--;
	}
	if (startPoints.first < 500 && startPoints.second < 500)
	{
		startPoints.first = startPoints.first++;
